...
public static class GitHubUtil
{
    private const string LatestReleaseUrl =
        @"https://api.github.com/repos/maximilian-hammerl/rsh-exporter/releases/latest";
    ...
    private static HttpClient GetOrCreateHttpClient()
    {
        if (_client == null)
        {
            ...
            _client.DefaultRequestHeaders.UserAgent.ParseAdd(
                $"maximilian-hammerl rsh-exporter ({Util.GetAppName()}@{Util.GetCurrentVersionAsString(4)})");
        }
    ...
    {
        private const string AssetName = "RSHExporter.exe";
